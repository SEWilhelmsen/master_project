...
markers <- FindAllMarkers(Mouse_3w_VCM)
...
barplot(go_results_VCM,showCategory = 20)
dotplot(go_results_VCM, showCategory = 20)
...
p1 <- DimPlot(Mouse_3w, group.by = "predicted.celltype.l2", label = TRUE, label.size = 3) + NoLegend()
p1

p2 <- DimPlot(Mouse_3w_VCM, group.by = "predicted.celltype.l2", label = TRUE, label.size = 3) + NoLegend()
p2
