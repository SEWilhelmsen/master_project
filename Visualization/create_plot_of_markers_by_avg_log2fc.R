...
# Subset the data to include only the marker genes
marker_genes <- gene_list$marker
...
mouse_vcm_marker_genes_avg_log2fc <- mouse_vcm_all_genes_avg_log2fc %>%
  filter(gene %in% marker_genes)

# Reshape data from wide to long format
mouse_vcm_marker_genes_avg_log2fc_long <- mouse_vcm_marker_genes_avg_log2fc %>%
  pivot_longer(cols = -gene, names_to = "time_point", values_to = "avg_log2fc")

# Set the correct order of time points.
mouse_vcm_marker_genes_avg_log2fc_long$time_point <- factor(mouse_vcm_marker_genes_avg_log2fc_long$time_point,
                                                         levels = c("avg_log2fc_3d",
                                                                    "avg_log2fc_1w",
                                                                    "avg_log2fc_3w"))

# Print to confirm
print(head(mouse_vcm_marker_genes_avg_log2fc_long))

# Create the ggplot for marker genes
ggplot(mouse_vcm_marker_genes_avg_log2fc_long, aes(x = time_point, y = avg_log2fc, group = gene, color = gene)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  scale_color_brewer(palette = "Set1") +
  xlab("Time Point") +
  ylab("Average log 2Fold Change Expression") +
  ggtitle("Average log 2fc of Marker Genes Over Time") +
