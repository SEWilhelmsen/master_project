...
load("Y:/Silje/snRNAseq/3weeks_sham_AB/Mouse_3w.RData")
load("Y:/Silje/snRNAseq/3weeks_sham_AB/top_genes.RData")

# 200 top genes
class(top_genes) # Make sure the input is of class data frame. 

# Convert the data frame to a matrix, as Heatmap expects a matrix input
data_matrix <- as.matrix(top_genes)


df.top <- top_genes[order(top_genes$log_pval, decreasing = TRUE),]
