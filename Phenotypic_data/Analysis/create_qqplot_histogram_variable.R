...
create_qqplot <- function(processed_data, variable, output_dir_distribution) {
  ...
  for (Timepoint in Timepoints) {
    ...
    variable_sham <- processed_data %>%
      filter(Condition == 'SHAM', Timepoint == Timepoint) %>%
      select(all_of(variable)) %>%
      unlist() %>%
      na.omit()
    
    variable_ab <- processed_data %>%
      filter(Condition == 'ORAB', Timepoint == Timepoint) %>%
      select(all_of(variable)) %>%
      unlist() %>%
      na.omit()
    ...
    print(paste("ORAB Data Length:", length(variable_ab)))
    ...
    qq_plot_sham <- ggqqplot(data.frame(variable = variable_sham), x = "variable", ggtheme = theme_bw()) + 
      ggtitle(paste("QQ Plot for sham at time point", Timepoint)) +
      labs(x = "Theoretical Quantiles", y = "Sample Quantiles")
    
    qq_plot_ab <- ggqqplot(data.frame(variable = variable_ab), x = "variable", ggtheme = theme_bw()) + 
      ggtitle(paste("QQ Plot for ORAB at time point", Timepoint)) +
      labs(x = "Theoretical Quantiles", y = "Sample Quantiles")
    ...
    # Save the plot
    sanitized_Timepoint <- gsub("[^a-zA-Z0-9]", "_", Timepoint)
    file_name <- file.path(output_dir_distribution, paste("qq_plot_", sanitized_Timepoint, "_", variable, ".png", sep = ""))
...
create_histogram <- function(processed_data, variable_of_interest, output_dir, binwidth = 0.2, fill_values = c("sham" = "white", "AB" = "black")) {
