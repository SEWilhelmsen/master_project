...
# # Inspect layers 
# Layers(mouse_3d_vcm_with_stress_status)
# 
# 
# # Extract Data
# data_matrix_3d <- GetAssayData(object = mouse_vcm_all_time_points_with_stress_status, slot = "data.Full Merge Project 3d", assay = "RNA")
# 
# 
# # Convert to Dense Matrix
# dense_matrix <- as.matrix(data_matrix_3d)

mouse_3d_vcm_with_stress_status <- readRDS("C:/Users/siljeew/Master_project/snRNAseq/tmp/mouse_3d_vcm_with_stress_status.Rds")

# Run Progeny Analysis
pathway_activity_scores <- progeny(mouse_3d_vcm_with_stress_status, scale = FALSE, organism = "Mouse", top = 500)
...
head(mouse_3d_vcm_with_stress_status@meta.data$Stress_Status)
conditions_df <- data.frame(Cell = colnames(mouse_3d_vcm_with_stress_status),
                            Stress_Status = mouse_3d_vcm_with_stress_status$Stress_Status,
                            Timepoint = mouse_3d_vcm_with_stress_status$Timepoint)
...
summarized_progeny_scores <- progeny_scores_df %>%
  group_by(Pathway, Stress_Status) %>%
...
activity_filtered_3d <- activity_filtered

# Import P-adjusted label
activity_filtered_3d <- activity_filtered_3d %>%
  left_join(updated_results_df %>% select(Pathway, Timepoint, p_adjusted_label), 
            by = c("Pathway", "Timepoint"))

# Remove NAs
p_adj_label_3d <- activity_filtered_3d %>%
  filter(p_adjusted_label != "ns") %>%
  mutate(y_position = difference + 0.01)

# Create the plot
progeny_3d <- ggplot(activity_filtered_3d, aes(x = Pathway, y = difference, fill = difference > 0)) +
  geom_bar(stat = "identity") +
  labs(x = "", 
       y = "Difference in activity score",
       title = "SHAM vs Stressed CM at 3 Days",
       fill = "Condition") +
  scale_fill_manual(
  ...
  geom_text(data = p_adj_label_3d, aes(x = Pathway, y = y_position, label = p_adjusted_label), 
            size = 10, color = "black") +
  ylim(-0.2, 0.25)
